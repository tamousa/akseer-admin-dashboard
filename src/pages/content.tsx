...
const BLOG_CATEGORIES: Record<BlogCategory, { label: string; color: string }> = {
  ...
  mental:    { label: "الصحة النفسية",          color: "bg-purple-100 text-purple-700" },
...
function BlogEditor({
  ...
  return (
    <div className="bg-white rounded-2xl border shadow-sm overflow-hidden" dir="rtl">
      {/* Header */}
      <div className="bg-gradient-to-l from-[#7C2D97]/10 to-purple-50 px-6 py-4 border-b flex items-center justify-between">
        <h2 className="font-bold text-[#1e0a2e] text-xl flex items-center gap-2">
          <BookOpen className="w-5 h-5 text-[#7C2D97]" />
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="space-y-2">
          <label className="text-sm font-semibold text-[#1e0a2e]">تصنيف المدونة *</label>
            ...
            {(Object.entries(BLOG_CATEGORIES) as [BlogCategory, { label: string; color: string }][]).map(([key, val]) => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm font-medium border-2 transition-all ${
                  category === key
                    ? "border-[#7C2D97] bg-[#EDE9FE] text-[#7C2D97]"
        ...
        {/* Title */}
        <div className="space-y-2">
          <label className="text-sm font-semibold text-[#1e0a2e]">عنوان المدونة *</label>
          <Input
            value={title}
            onChange={e => setTitle(e.target.value)}
            placeholder="أدخل عنوان المدونة..."
            className="text-lg font-semibold border-gray-200 focus:border-[#7C2D97] h-12"
          />
        </div>

        {/* Images */}
        <div className="space-y-3">
          <label className="text-sm font-semibold text-[#1e0a2e] flex items-center gap-2">
            <ImageIcon className="w-4 h-4 text-[#7C2D97]" />
            الصور
          </label>
          <div className="flex flex-wrap gap-3">
            ...
            <button
              ...
              className="w-28 h-28 rounded-xl border-2 border-dashed border-gray-200 flex flex-col items-center justify-center gap-2 text-gray-400 hover:border-[#7C2D97] hover:text-[#7C2D97] transition-colors"
            >
        ...
        {/* Content */}
        <div className="space-y-2">
          <label className="text-sm font-semibold text-[#1e0a2e] flex items-center justify-between">
          ...
          <Textarea
            ...
            className="min-h-[320px] text-base leading-relaxed border-gray-200 focus:border-[#7C2D97] resize-y"
          />
        </div>
        ...
        <div className="space-y-3">
          <label className="text-sm font-semibold text-[#1e0a2e] flex items-center gap-2">
            <Link2 className="w-4 h-4 text-[#7C2D97]" />
            ...
            <div className="space-y-2">
              {links.map((link, i) => (
                <div key={i} className="flex items-center gap-3 bg-gray-50 rounded-lg px-4 py-2.5">
                  <Link2 className="w-4 h-4 text-[#7C2D97] shrink-0" />
                  <span className="text-sm font-medium text-[#1e0a2e]">{link.label}</span>
        ...
        <div className="space-y-3">
          <button
            ...
            className="flex items-center gap-2 text-sm font-semibold text-[#7C2D97] hover:text-[#5a1f6e]"
          >
          ...
          {showSchedule && (
            <Input
              ...
              className="w-72 border-[#7C2D97]/30 focus:border-[#7C2D97]"
            />
          )}
        </div>
        ...
        <div className="flex items-center gap-3 pt-4 border-t">
          <Button
            ...
            className="bg-[#7C2D97] hover:bg-[#5a1f6e] text-white gap-2 px-6"
          >
          ...
          {showSchedule && (
            <Button
              ...
              variant="outline"
              className="border-[#7C2D97] text-[#7C2D97] gap-2 px-6"
            >
              <Clock className="w-4 h-4" />
              جدولة النشر
            </Button>
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-2 flex-wrap">
          <button
            ...
            className={`px-4 py-1.5 rounded-full text-sm font-medium transition-all ${filterCat === "all" ? "bg-[#7C2D97] text-white" : "bg-white border text-gray-600 hover:border-[#7C2D97]"}`}
          >
            الكل ({blogs.length})
          </button>
          {(Object.entries(BLOG_CATEGORIES) as [BlogCategory, { label: string; color: string }][]).map(([key, val]) => {
            ...
            return (
              <button
                ...
                className={`px-4 py-1.5 rounded-full text-sm font-medium transition-all ${filterCat === key ? "bg-[#7C2D97] text-white" : "bg-white border text-gray-600 hover:border-[#7C2D97]"}`}
              >
                {val.label} ({count})
              </button>
            );
        ...
        <Button
          ...
          className="bg-[#7C2D97] hover:bg-[#5a1f6e] text-white gap-2"
        >
          <Plus className="w-4 h-4" />
          مدونة جديدة
        </Button>
          ...
          return (
            <Card key={post.id} className="border-gray-100 shadow-sm overflow-hidden hover:shadow-md transition-shadow">
              ...
              <CardContent className="p-5 space-y-3">
                ...
                <h3 className="font-bold text-[#1e0a2e] text-lg leading-snug">{post.title}</h3>
                ...
                <div className="flex items-center gap-2 pt-2 border-t">
                  <Button variant="outline" size="sm" onClick={() => setEditing(post)} className="gap-1.5 flex-1 text-[#7C2D97] border-[#7C2D97]/30">
    ...
    <div className="space-y-6" dir="rtl">
          ...
          return (
            <button
              ...
              className={`p-4 rounded-2xl border-2 text-right transition-all ${
                activeType === key
                  ? "border-[#7C2D97] bg-[#EDE9FE]"
                  : "border-gray-100 bg-white hover:border-[#7C2D97]/30"
              }`}
            >
              <div className="text-2xl mb-1">{val.icon}</div>
              <div className={`text-sm font-semibold ${activeType === key ? "text-[#7C2D97]" : "text-[#1e0a2e]"}`}>{val.label}</div>
              <div className={`text-xs mt-0.5 ${activeType === key ? "text-[#7C2D97]/70" : "text-gray-400"}`}>{count} نصيحة</div>
            </button>
          );
        })}
      </div>
      ...
      <Card className="border-gray-100 shadow-sm">
        <CardContent className="p-5 space-y-4">
          <h3 className="font-bold text-[#1e0a2e] flex items-center gap-2">
            <Lightbulb className="w-5 h-5 text-[#7C2D97]" />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Textarea
                ...
                className="min-h-[100px] border-gray-200 focus:border-[#7C2D97]"
              />
            </div>
            <div className="space-y-2">
              <label className="text-sm font-semibold flex items-center gap-2">
          ...
          <div className="flex gap-3 pt-2">
            {editingId ? (
              <>
                <Button onClick={handleUpdate} className="bg-[#7C2D97] hover:bg-[#5a1f6e] text-white gap-2">
                  <Save className="w-4 h-4" />
                  تحديث
                </Button>
              ...
              <Button onClick={handleAdd} className="bg-[#7C2D97] hover:bg-[#5a1f6e] text-white gap-2">
                <Plus className="w-4 h-4" />
                إضافة النصيحة
              </Button>
            )}
      ...
      {typeTips.length === 0 ? (
        ...
        <div className="space-y-3">
          {typeTips.map((tip, i) => (
            <Card key={tip.id} className={`border-gray-100 shadow-sm transition-all ${editingId === tip.id ? "ring-2 ring-[#7C2D97]" : ""}`}>
              <CardContent className="p-5">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 space-y-1.5">
                    <div className="flex items-center gap-2">
                      <span className="w-7 h-7 bg-[#EDE9FE] text-[#7C2D97] rounded-full flex items-center justify-center text-sm font-bold shrink-0">
                        {i + 1}
                      </span>
                      <p className="text-[#1e0a2e] font-medium leading-relaxed">{tip.textAr}</p>
                    </div>
                    {tip.textEn && (
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <Button variant="ghost" size="sm" onClick={() => startEdit(tip)} className="text-[#7C2D97] hover:bg-[#EDE9FE]">
    ...
    <Tabs defaultValue="legal" dir="rtl">
      <TabsList className="bg-white border rounded-xl p-1 shadow-sm w-full justify-start h-auto flex-wrap">
        {Object.entries(sections).map(([k, label]) => (
          <TabsTrigger key={k} value={k} className="data-[state=active]:bg-[#EDE9FE] data-[state=active]:text-[#7C2D97] rounded-lg px-6 py-2.5">
            {label}
          </TabsTrigger>
        ))}
      </TabsList>
            ...
            return (
              <Card key={item.key} className="border-gray-200 shadow-sm overflow-hidden">
                <div className="bg-gray-50 px-6 py-3 border-b flex justify-between items-center">
                  <h3 className="font-bold text-[#1e0a2e] text-lg">{item.titleAr}</h3>
                </div>
                <CardContent className="p-6 space-y-6">
                  ...
                  <div className="flex justify-end pt-2 border-t">
                    <Button
                      ...
                      disabled={!changed}
                      className={changed ? "bg-[#7C2D97] hover:bg-[#5a1f6e] text-white gap-2" : "bg-gray-100 text-gray-400 gap-2"}
                    >
...
export default function Content() {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500" dir={isRTL ? "rtl" : "ltr"}>
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-[#1e0a2e]">{t("محتوى التطبيق", "App Content")}</h1>
      ...
      <Tabs defaultValue="blog" className="w-full" dir={isRTL ? "rtl" : "ltr"}>
        <TabsList className="bg-white border rounded-xl p-1 shadow-sm w-full justify-start h-auto flex-wrap gap-1">
          <TabsTrigger value="blog" className="data-[state=active]:bg-[#EDE9FE] data-[state=active]:text-[#7C2D97] rounded-lg px-6 py-2.5 gap-2">
            <BookOpen className="w-4 h-4" />
            {t("المدونة", "Blog")}
          </TabsTrigger>
          <TabsTrigger value="tips" className="data-[state=active]:bg-[#EDE9FE] data-[state=active]:text-[#7C2D97] rounded-lg px-6 py-2.5 gap-2">
            <Lightbulb className="w-4 h-4" />
            {t("النصائح اليومية", "Daily Tips")}
          </TabsTrigger>
          <TabsTrigger value="static" className="data-[state=active]:bg-[#EDE9FE] data-[state=active]:text-[#7C2D97] rounded-lg px-6 py-2.5 gap-2">
